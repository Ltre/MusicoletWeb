...
func insertSnapshot(ctx context.Context, tx *sql.Tx, sid int64, s domain.Snapshot) error {
	...
	for p, c := range s.CurrentCounts {
		_, _ = tx.ExecContext(ctx, "INSERT OR REPLACE INTO snapshot_current_counts VALUES(?,?,?,?,?)", sid, p, c.Week, c.Month, c.Year)
	}
	for k, v := range s.Settings {
		_, _ = tx.ExecContext(ctx, "INSERT OR REPLACE INTO snapshot_settings VALUES(?,?,?)", sid, k, string(v))
	}
...
func replaceWorking(ctx context.Context, tx *sql.Tx, s domain.Snapshot) error {
	// Runtime playback is server-owned. Preserve the active queue by business name
	// because working queue IDs are recreated on every import. On the first import,
	// seed it from Musicolet's S0_CPQ snapshot value instead.
	var activeQueueName string
	_ = tx.QueryRowContext(ctx, "SELECT COALESCE(q.name,'') FROM runtime_playback_state r LEFT JOIN working_queues q ON q.id=r.queue_id WHERE r.singleton=1").Scan(&activeQueueName)
	...
	for _, t := range []string{"working_playlist_items", "working_playlists", "working_queue_items", "queue_playback_state", "working_queues", "working_favorites", "working_settings", "working_period_counts", "working_current_counts", "working_songs"} {
	...
	for i, q := range s.Queues {
		...
		if (activeQueueName != "" && q.Name == activeQueueName) || (activeQueueName == "" && i == s.CurrentQueueIndex) {
			activeQueueID = qid
		}
		for j, path := range q.Paths {
	...
	if activeQueueID != 0 {
		_, _ = tx.ExecContext(ctx, "UPDATE runtime_playback_state SET queue_id=?,updated_at=? WHERE singleton=1", activeQueueID, db.NowMS())
	} else {
		_, _ = tx.ExecContext(ctx, "UPDATE runtime_playback_state SET queue_id=NULL,playing=0,updated_at=? WHERE singleton=1", db.NowMS())
	}
	for p := range s.Favorites {
		_, _ = tx.ExecContext(ctx, "INSERT OR IGNORE INTO working_favorites VALUES(?)", p)
	}
	for k, v := range s.Settings {
		_, _ = tx.ExecContext(ctx, "INSERT OR REPLACE INTO working_settings VALUES(?,?)", k, string(v))
	}
	for k, m := range s.PeriodCounts {
		for p, c := range m {
	...
	for p, c := range s.CurrentCounts {
		_, _ = tx.ExecContext(ctx, "INSERT OR REPLACE INTO working_current_counts(path,week_count,month_count,year_count) VALUES(?,?,?,?)", p, c.Week, c.Month, c.Year)
	}
	return nil
}
...
func queueNames(a []domain.Queue) []string {
