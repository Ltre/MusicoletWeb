...
func (s *Service) recordChangeLocked(ctx context.Context, targetType, targetKey, operation string, before, after any, targets ...[2]string) error {
	...
	_, _, vno, _ := s.Store.LatestVersion(ctx)
	var cid int64
	if err := s.Store.Tx(ctx, func(tx *sql.Tx) error {
		r, e := tx.ExecContext(ctx, "INSERT INTO server_changes(base_version_id,target_type,target_key,operation,before_json,after_json,git_commit,active,created_at) SELECT id,?,?,?,?,?,NULL,1,? FROM musicolet_versions WHERE version_no=?", targetType, targetKey, operation, string(b), string(a), db.NowMS(), vno)
		if e != nil {
			return e
		}
		cid, _ = r.LastInsertId()
		if cid == 0 {
			return errors.New("server changes require an imported Musicolet base version")
		}
		all := append([][2]string{{targetType, targetKey}}, targets...)
		for _, t := range all {
			_, _ = tx.ExecContext(ctx, "INSERT OR IGNORE INTO change_targets(change_id,target_type,target_key) VALUES(?,?,?)", cid, t[0], t[1])
			if t[0] == "song" {
				_, _ = tx.ExecContext(ctx, "UPDATE working_songs SET has_server_changes=1 WHERE path=?", t[1])
			}
		}
		return nil
	}); err != nil {
	...
	state, _ := s.loadWorking(ctx)
	sj, _ := json.Marshal(state)
	parent := s.Git.Head("refs/heads/main")
	commit, e := s.Git.CommitJSON("refs/heads/main", operation, sj, parent)
	if e != nil {
		return e
	}
	_, e = s.Store.DB.ExecContext(ctx, "UPDATE server_changes SET git_commit=? WHERE id=?", commit, cid)
	return e
}

// ReconcileGit repairs the auditable Git side after a process crash that occurred
// after SQLite accepted a server change but before its Git commit was recorded.
...
func (s *Service) ReconcileGit(ctx context.Context) error {
	...
	// Detect a crash in the small window between a Working State mutation and its change row.
...
// Mutations used by the initial Web UI/API.
