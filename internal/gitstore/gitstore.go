...
func Open(dir string) (*Store, error) {
	...
	if _, err := os.Stat(filepath.Join(dir, "HEAD")); os.IsNotExist(err) {
		if _, err = s.run(nil, "init", "--bare", dir); err != nil {
			return nil, err
		}
	}
	return s, nil
...
func (s *Store) Parents(commit string) ([]string, error) {
	out, err := s.run(nil, "rev-list", "--parents", "-n", "1", commit)
	if err != nil {
		return nil, err
	}
	fields := strings.Fields(string(out))
	if len(fields) == 0 {
		return nil, fmt.Errorf("git commit %s has no rev-list output", commit)
	}
	return append([]string(nil), fields[1:]...), nil
}

func (s *Store) CommitMatches(commit string, data []byte, parents ...string) (bool, error) {
	state, err := s.ReadState(commit)
	if err != nil {
		return false, err
	}
	if !bytes.Equal(bytes.TrimSpace(state), bytes.TrimSpace(data)) {
		return false, nil
	}
	got, err := s.Parents(commit)
	if err != nil {
		return false, err
	}
	want := make([]string, 0, len(parents))
...
func (s *Store) CommitJSON(ref, msg string, data []byte, parents ...string) (string, error) {
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		return "", fmt.Errorf("git mktree: %w: %s", err, out)
	}
...
func (s *Store) hashObject(data []byte) (string, error) {
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		return "", fmt.Errorf("git hash-object: %w: %s", err, out)
	}
